  ...
  return (
    <section className="mt-8 rounded-2xl border border-slate-800 bg-slate-900/80 p-5 shadow-lg shadow-slate-950/60">
      <header className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-baseline sm:justify-between">
        <div>
          <Text variant="subtitle" className="text-slate-100">
            Tygodniowy ranking – Kółko i krzyżyk (wygrane z botem)
          </Text>
          <Text variant="body-small">
            Liczy się łączna liczba zwycięstw z ostatnich 7 dni. Jeden gracz może
            zająć tylko jedno miejsce w TOP 10 – im więcej wygranych, tym wyżej.
          </Text>
        </div>
      ...
      {!isLoading && !isError && (!data || data.items.length === 0) && (
        <Text variant="body-small">
          Brak zapisanych zwycięstw w tym tygodniu. Wygraj z botem, aby otworzyć
          tabelę!
        </Text>
      )}
          ...
          <table className="min-w-full text-left text-sm text-slate-100">
            <thead>
              <tr className="border-b border-slate-800 text-xs uppercase tracking-wide text-slate-400">
                ...
                <th className="py-2 pr-4 text-right">Liczba zwycięstw</th>
